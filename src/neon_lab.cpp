...
std::vector<BenchmarkResult> run_all_benchmarks() {
    ...
    double start_log = 2.0;
    double end_log = 6.0;
    int total_points = 41; 
    double step = (end_log - start_log) / (total_points - 1);

    std::mt19937 gen(42);
    std::uniform_int_distribution<int32_t> dist(-1000, 1000);

    for (int i = 0; i < total_points; ++i) {
        size_t n = static_cast<size_t>(pow(10.0, start_log + i * step));
        if (n < 1) n = 1;

        size_t aligned_n = (n + 3) & ~3;
        size_t alloc_size = aligned_n * sizeof(int32_t);
        if (alloc_size % 16 != 0) alloc_size = (alloc_size + 15) & ~15;

        int32_t* data = static_cast<int32_t*>(aligned_alloc(16, alloc_size));
        if (!data) continue;

        for (size_t j = 0; j < n; ++j) data[j] = dist(gen);
        for (size_t j = n; j < aligned_n; ++j) data[j] = 0;
