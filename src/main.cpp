...
void DrawPerformanceGraph(const std::vector<BenchmarkResult>& results, float max_time) {
    ...
    float margin = 40.0f;
    ImVec2 plot_p0 = ImVec2(canvas_p0.x + margin, canvas_p0.y + 10.0f);
    ImVec2 plot_p1 = ImVec2(canvas_p1.x - 10.0f, canvas_p1.y - margin);
    ...
    draw_list->AddText(ImVec2(plot_p1.x - 50, plot_p1.y + 5), IM_COL32(200, 200, 200, 255), "Size (N)");
    draw_list->AddText(ImVec2(plot_p0.x - 35, plot_p0.y - 5), IM_COL32(200, 200, 200, 255), "Time (ms)");

    auto get_pos = [&](int index, float time) {
        float x_ratio = (float)index / (float)(results.size() - 1);
        float y_ratio = time / max_time;
        return ImVec2(plot_p0.x + x_ratio * plot_w, plot_p1.y - y_ratio * plot_h);
    };

    for (int i = 0; i < (int)results.size(); i++) {
        ImVec2 p = get_pos(i, 0);
        char buf[32];
        sprintf(buf, "%zu", results[i].array_size);
        draw_list->AddText(ImVec2(p.x - 15, plot_p1.y + 5), IM_COL32(150, 150, 150, 255), buf);
    }

    for (int i = 0; i < (int)results.size() - 1; i++) {
        ImVec2 p1_s = get_pos(i, results[i].scalar_time_ms);
        ImVec2 p2_s = get_pos(i + 1, results[i+1].scalar_time_ms);
        draw_list->AddLine(p1_s, p2_s, IM_COL32(255, 100, 100, 255), 2.0f);

        ImVec2 p1_n = get_pos(i, results[i].neon_time_ms);
        ImVec2 p2_n = get_pos(i + 1, results[i+1].neon_time_ms);
        draw_list->AddLine(p1_n, p2_n, IM_COL32(100, 255, 100, 255), 2.0f);
    }

    for (int i = 0; i < (int)results.size(); i++) {
        ImVec2 p_s = get_pos(i, results[i].scalar_time_ms);
        draw_list->AddCircleFilled(p_s, 4.0f, IM_COL32(255, 100, 100, 255));
        
        ImVec2 p_n = get_pos(i, results[i].neon_time_ms);
